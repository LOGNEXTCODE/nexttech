...
- [ ] **Comillas tipográficas curvas, nunca rectas** — entrecomillados de
      contenido con ‘ ’ (simples) y “ ” (dobles) bien orientadas en ES/EN;
      FR mantiene sus « » estándar, igual que los textos fijos que ya las usan
      (cookies, entradillas aprobadas). Las « » angulares en contenido ES/EN se
      probaron y DESCARTARON el 1-sep-2026 (a Miguel no le gustan en la web).
      `generator.py` normaliza rectas → curvas al generar (`_normalize_quotes`),
      pero verificar en cada edición, títulos incluidos: las #02/#03/#04
      nacieron con rectas y hubo que parchearlas a mano (ojo con lo que
      introduce `translator.py` en EN/FR: ese paso NO normaliza).
